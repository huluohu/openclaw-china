...
export async function getAccessToken(account: ResolvedWecomAppAccount): Promise<string> {
  ...
  const apiHost = account.apiHost || "https://qyapi.weixin.qq.com";
  const url = `${apiHost}/cgi-bin/gettoken?corpid=${encodeURIComponent(account.corpId)}&corpsecret=${encodeURIComponent(account.corpSecret)}`;
...
export async function downloadWecomMediaToFile(
  ...
  try {
    if (isHttp) {
      ...
      const apiHost = account.apiHost || "https://qyapi.weixin.qq.com";
      const url = `${apiHost}/cgi-bin/media/get?access_token=${encodeURIComponent(token)}&media_id=${encodeURIComponent(safeMediaId)}`;
...
export async function sendWecomAppMessage(
  ...
  const resp = await fetch(
    `${apiHost}/cgi-bin/message/send?access_token=${encodeURIComponent(token)}`,
    {
      method: "POST",
...
export async function uploadImageMedia(
  ...
  const apiHost = account.apiHost || "https://qyapi.weixin.qq.com";
  const resp = await fetch(
    `${apiHost}/cgi-bin/media/upload?access_token=${encodeURIComponent(token)}&type=image`,
    {
      method: "POST",
      body: body,
      headers: {
...
export async function uploadVoiceMedia(
  ...
  const apiHost = account.apiHost || "https://qyapi.weixin.qq.com";
  const resp = await fetch(
    `${apiHost}/cgi-bin/media/upload?access_token=${encodeURIComponent(token)}&type=voice`,
    {
      method: "POST",
      body: body,
      headers: {
...
export async function uploadMedia(
  ...
  const apiHost = account.apiHost || "https://qyapi.weixin.qq.com";
  const resp = await fetch(
    `${apiHost}/cgi-bin/media/upload?access_token=${encodeURIComponent(token)}&type=${type}`,
    {
      method: "POST",
      body: body,
      headers: {
